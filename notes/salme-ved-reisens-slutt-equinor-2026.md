Title: Salme ved reisen slutt
Location: Rogaland, lokalcamp
Subtitle: Om å gå fra bordet hos Equinor

Slides:
# Industri 4.0
Bakgrunn: 
    industri 4.0, fra Unsplash
notes: Jeg har jobbet på (og av) prosjektet over halvannet år. Fantastisk kjekke folk, godt og trygt arbeidsmiljø tross et svært distribuert team.

# Oppgaven
bakgrunn:
    Photo by <a href="https://unsplash.com/@mrsunflower94?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Matteo Vistocco</a> on <a href="https://unsplash.com/photos/people-riding-boat-on-body-of-water-Dph00R2SwFo?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
notes:
    Vi ønsker å bygge et interoperabelt fundament for data i Equinor.

    smak på _interoperabelt_. Det betyr... 

# TDI & EPN
Bakgrunn:
    Photo by <a href="https://unsplash.com/@karohubert?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Karollyne Videira Hubert</a> on <a href="https://unsplash.com/photos/text-HETSjinNkT8?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>

Text:
    TDI & EPN (eller; en reise i TBF-er)

    

notes:
    Technology, digital and innovation
    Exploration and production Norway

    Det er _langt_ fra TDI til EPN. Det er langt fra Kårstø til Fornebu. 

# CoPilot og Fellesskriving
Bakgrunn:
    Photo by <a href="https://unsplash.com/@markuswinkler?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Markus Winkler</a> on <a href="https://unsplash.com/photos/black-and-white-typewriter-on-green-table-_nvKjg0aliA?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>

notes:


notes: 
    Siden 
# Hva jeg sier når jeg ser takk for meg
Bakgrunn: 
    bilde av folk som sier adjø, fra Unsplash
notes: prosjektet mitt er lagt ned. vi leverer rapporten vår i mars, og jobber for å få arbeidet løftet over på andre organisatoriske hender før høsten starter. 

# Karriere og oppdrag
Bakgrunn: 
    bilde av en stige, fra Unsplash
notes: 
    min første økonomisjef i miin første konsulentjobb sa "ingen klarer å holde følge med konsulenter — etter fem år flyr dere forbi folk som sitter i faste stillinger rundt i norge"

# CV-skriving og sannheten
Bakgrunn:
    bilde av en sti gjennom skogen, fra Unsplash
notes:
    Vi bruker CV-en vår til å fortelle hva vi kan, og må bruke størrelsene "Prosjekt" og "År med erfaring" for å få dette frem.

# Fleksibilitet og vårt bidrag
bakgrunn:
    bilde av norsk by, fra Unsplash (eller en annen egnet plass)
notes:
    - https://www.finansavisen.no/teknologi/2024/10/23/8195420/vi-trenger-flere-konsulenter-ikke-faerre?zephr_sso_ott=wJ3pLB#:~:text=Rundt%20130.000%20personer%20tilbyr%20konsulenttjenester,%C3%A5rlig%2C%20if%C3%B8lge%202019%2Dtall.
    - https://www.linkedin.com/posts/erna-solberg-18516b7_jeg-skulle-%C3%B8nske-alle-var-enige-i-at-det-activity-7264543592227000323-d2h9/?originalSubdomain=no


